...
class DeleteCompany extends React.Component{
    ...
    deleteCompany = () =>{
        ...
        axios.delete(`http://localhost:8081/deleteCompany/${companyId}`).then((response)=>{
            this.setState({res : response.data.message})
            window.confirm(response.data.message)
        });
    } 
    componentDidMount(){
        axios.get('http://localhost:8081/getAllCompany').then((response)=>{
                                ...
                                <tbody>
                                    {
                                        this.state.companies.map(company=>
                                            <tr key = {company.id}>
                                                ...
                                                <td>
                                                    {/* <Link className="btn text-primary" to="/updateCompany">
